...
## Amendment: Commit Graph (read-only)

Исходный запрет «без собственного graph UI» частично снят пользователем: добавлен read-only граф коммитов в отдельной Webview-панели (`Show Commit Graph`). Границы:

- граф только читает (`git log`, `git show`, `git diff --name-status`) — никакого merge/conflict UI, этот запрет в силе;
- lanes-алгоритм и парсинг живут в `src/graph/` как чистые модули с тестами, панель — в `src/ui/graphPanel.ts`;
- действия из графа (checkout, compare, copy) переиспользуют существующие use-cases, новых git-мутаций панель не вводит;
- панель обновляется через общий refresh-путь (включая watcher); лимит 500 коммитов.
